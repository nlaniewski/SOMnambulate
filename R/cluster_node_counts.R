...
##
node_counts_long <- function(dat,node.col='node',sample.col='sample.id',
                             sample.split.cols=c('subject','visit','condition','batch','batch.date')){
  if(!node.col %in% names(dat)){
    stop(paste("Specified node column name not found:",node.col))
  }
  ...
  data.table::setkeyv(dat, c(sample.col, node.col))
  #dat.N.node<-dat[data.table::CJ(sample, node, unique = TRUE), .N, by = .EACHI]
  dat.N.node<-dat[data.table::CJ(get(sample.col), get(node.col), unique = TRUE), .N, by = .EACHI]
  #dat.N.node[, prop := N/sum(N) * 100, by = sample]
  dat.N.node[, 'prop' := get('N')/sum(get('N')) * 100, by = sample.col]
  #dat.N.node[, per1million := N*(1E6/sum(N)), by = sample]
  dat.N.node[, 'per1million' := get('N')*(1E6/sum(get('N'))), by = sample.col]
  ##
  if(!all(sapply(strsplit(dat.N.node[[sample.col]],"_"),length)==length(sample.split.cols))){
    stop("Splitting 'sample name' string by '_' results in a length != to length of 'sample.split.cols'")
  }
  dat.N.node[,(sample.split.cols) := data.table::tstrsplit(get(sample.col),"_")]
  return(dat.N.node)
}
